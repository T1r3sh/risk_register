...
class editMeasureUIDialog(object):
    def setupUi(self, Dialog, measure_id):
        ...
        self.pm_data_loader(measure_id)

        # data_placeholder
        self.risk_data_placeholder()
        self.pm_data_placeholder()
    ...
    def retranslateUi(self, Dialog):
        ...
        Dialog.setWindowTitle(_translate("Dialog", "Изменение меры"))
    ...
    def pm_data_loader(self, pm_id):
        qc = queryComposer("prevention_measures")
        self.current_pm_data = qc.select_query(
            conditions=[
                {
    ...
    def pm_data_placeholder(self):
        self.name_field.setText(self.current_pm_data["prevention_measure_name"])
        self.details_field.setText(self.current_pm_data["prevention_measure_details"])
        self.risk_combobox.setCurrentIndex(
            list(self.risk_data_id_ref.keys()).index(self.current_pm_data["risk_id"])
        )
        self.status_combobox.setCurrentIndex(
            1 if self.current_pm_data["prevention_measure_status"] else 0
        )
        if self.current_pm_data["prevention_measure_status"]:
            tmp = list(
                map(
                    int,
                    str(self.current_pm_data["prevention_measure_date"]).split("-"),
                )
            )
            self.date_field.setDate(QtCore.QDate(*tmp))
    ...
    def confirm_button_slot(self, Dialog: QtWidgets.QDialog = None) -> None:
        qc = queryComposer("prevention_measures")
        values = {}
        values["prevention_measure_id"] = self.current_pm_data["prevention_measure_id"]
