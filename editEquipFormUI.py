...
class editEquipUIDIalog(object):
    def setupUi(self, Dialog, current_equip_id):
        Dialog.setObjectName("Dialog")
        ...
        # self.equipSensorLabel = QtWidgets.QLabel(parent=self.gridLayoutWidget)
        # self.equipSensorLabel.setObjectName("equipSensorLabel")
        # self.gridLayout.addWidget(self.equipSensorLabel, 7, 0, 1, 1)
        # self.equipSensorQListWidget = QtWidgets.QListWidget(
        #     parent=self.gridLayoutWidget
        # )
        # self.equipSensorQListWidget.setObjectName("equipSensorQListWidget")
        # self.gridLayout.addWidget(self.equipSensorQListWidget, 7, 1, 1, 1)
        # self.changeSensorToolButton = QtWidgets.QToolButton(
        #     parent=self.gridLayoutWidget
        # )
        # self.changeSensorToolButton.setObjectName("changeSensorToolButton")
        # self.gridLayout.addWidget(self.changeSensorToolButton, 7, 2, 1, 1)
    ...
    def retranslateUi(self, Dialog):
        _translate = QtCore.QCoreApplication.translate
        Dialog.setWindowTitle(_translate("Dialog", "Изменение оборудования"))
        ...
        self.confirmButton.setText(_translate("Dialog", "Изменить"))
        self.cancelButton.setText(_translate("Dialog", "Отмена"))

    # data loaders
    def equip_data_loader(self, equip_id):
        qc = queryComposer("equipment")
        raw_equip_data = qc.select_query(
            conditions=[
                {
        ...
        self.current_equipment_data = raw_equip_data[0]
    ...
    def equip_data_placeholder(self):
        self.equip_data_loader(self.current_equip_id)
        tmp = self.current_equipment_data
        self.equipNameField.setText(tmp["equipment_name"])
        self.equipDesignationField.setText(tmp["equipment_designation"])
        self.equipManufactField.setText(tmp["equipment_manufacturer_name"])
        self.equipLifetimeField.setText(str(tmp["equipment_average_lifetime"]))
        self.equipDetailsField.setText(tmp["equipment_details"])
        self.equipTypeComboBox.setCurrentIndex(
            list(self.equip_type_id_ref.keys()).index(tmp["equipment_type_id"])
        )
        self.equipStatusComboBox.setCurrentIndex(1 if tmp["equipment_status"] else 0)
        self.equipInstallationDateField.setEnabled(False)
        if tmp["equipment_status"]:
            self.equipInstallationDateField.setEnabled(True)
            d_tmp = list(
                map(
                    int,
                    str(
                        self.current_equipment_data["equipment_installation_date"]
                    ).split("-"),
                )
            )
            self.equipInstallationDateField.setDate(QtCore.QDate(*d_tmp))
    ...
    def confirm_button_slot(self, Dialog):
        qc = queryComposer("equipment")
        self.current_equipment_data["equipment_name"] = self.equipNameField.text()
        self.current_equipment_data[
            "equipment_designation"
        ] = self.equipDesignationField.text()
        self.current_equipment_data[
            "equipment_details"
        ] = self.equipDetailsField.toPlainText()
        self.current_equipment_data["equipment_status"] = (
            self.equipStatusComboBox.currentIndex() == 1
        )
        self.current_equipment_data["equipment_average_lifetime"] = str(
            self.equipLifetimeField.text()
        )
        self.current_equipment_data[
            "equipment_manufacturer_name"
        ] = self.equipManufactField.text()
        self.current_equipment_data["equipment_type_id"] = str(
            list(self.equip_type_id_ref.keys())[self.equipTypeComboBox.currentIndex()]
        )
        ...
        qc.update_query(self.current_equipment_data)
        qc.close_connection()
        Dialog.close()
