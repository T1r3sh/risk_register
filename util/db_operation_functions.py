...
#         logging.warning(f"In{func}Elapsed time: {(end_time - start_time):.6f} seconds")
#         return result
...
class queryComposer:
    ...
    def __init__(self, table_name: str, conn: psycopg2.connect = None) -> None:
        if conn is None:
    ...
    def delete_query(self, conditions):
        """
        Запрос на удаление
        ...
        if conditions is not None:
            ...
            while conditions:
                ...
                if not tmp_cond["key_name"].endswith("_id"):
                    tmp_cond["key_value"] = f"'{tmp_cond['key_value']}'"
