# Form implementation generated from reading ui file 'addSensorUIDIalog.ui'
#
...
class addUnitUIDialog(object):
    ...
    def confirm_button_slot(self, Dialog: QtWidgets.QDialog = None):
        ...
        logging.warning(qc.select_query())
        qc.close_connection()
        Dialog.close()
...
class addSensorUIDIalog(object):
    def setupUi(self, Dialog: QtWidgets.QDialog, current_equip_id: int):
        Dialog.setObjectName("Dialog")
        ...
        self.equipmentComboBox.setCurrentIndex(
            list(self.equipment_id_ref.keys()).index(current_equip_id)
        )
    ...
    def retranslateUi(self, Dialog):
        _translate = QtCore.QCoreApplication.translate
        Dialog.setWindowTitle(_translate("Dialog", "Добавления датчика"))
        ...
        self.sensorConfirmButton.setText(_translate("Dialog", "Добавить"))
    ...
    def sensor_unit_placeholder(self):
        logging.warning(f"comboBoxCurrentText {self.sensorTypeComboBox.currentText()}")
    ...
    def confirm_button_slot(self, Dialog: QtWidgets.QDialog = None):
        ...
        values = {}
        values["sensor_name"] = self.sensorNameField.text()
        values["limit_mode_value"] = self.sensorLimitField.text()
        values["sensor_average_lifetime"] = self.sensorLifetimeField.text()
        values["sensor_type_id"] = list(self.sensor_type_id_ref.keys())[
            list(self.sensor_type_id_ref.values()).index(
                self.sensorTypeComboBox.currentText()
            )
        ]
        values["unit_id"] = list(self.sensor_unit_id_ref.keys())[
            list(self.sensor_unit_id_ref.values()).index(
                self.sensorUnitComboBox.currentText()
            )
        ]
        values["equipment_id"] = list(self.equipment_id_ref.keys())[
